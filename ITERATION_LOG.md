...
## 2026-09-19：判断影响真实账本回放

### FLG-ITER-20260919-10：真实关系图未证明需要扩张 impact 状态机 [fixed — evidence]

**场景**：`flg impact` 已能沿显式关系生成只读复核候选，但真实反转下的误报情况
尚未单独记录。当前 FlowGrid 正式账本有 48 条决策和 3 条显式关系，覆盖
`supersedes`、`supports` 与 `depends_on`。

**处理**：冻结“决策的证据、有效性或正式 standing 发生实质变化”这一输入语义，
分别以 D-037、D-044、D-047、D-048 为变化根运行只读 impact，并人工核对最短
路径。未修改任何正式决策状态或关系。

**结果**：4 个变化根共产生 5 个复核候选实例；在冻结语义下，明显误报 0、漏报
0。D-047 变化时带出已被替代的 D-037 属于合理 Owner review：需要判断旧路线的
历史 standing 或 fallback 相关性，但不能自动恢复 D-037。

**边界**：样本只有 3 条边，没有 `conflicts_with`、多父依赖、循环或大型图噪声。
本轮不增加 `change_kind`、模型推断或自动状态迁移。完整记录见
`evals/results/judgment-impact-real-replay-20260919.md`。

## 2026-09-19：中文转述归因护栏

### FLG-ITER-20260919-09：`User:` 标签不能证明句中决定属于用户 [fixed — PR #54]

**场景**：关键词提取把 `User: 客户说他们已经决定采用方案 A` 识别为高置信候选，
后续又以 `source_actor: user` 输出。跨句的“客户发来结论。决定采用方案 A”也会
继承 `User:` 标签，造成同样的错误归因。这里的标签只证明谁提供了文本，无法证明
谁作出了句中的决定。

**处理**：新增确定性第三方转述护栏。用户转述客户、团队、领导等主体的说法、
要求或结论时 abstain；直接 `Client:` 发言保持有效；“客户建议 A，但我们现在
决定 B”这种明确的第一人称二次拍板仍生成候选。不尝试用关键词完成通用代词消解。

**验证**：新增同句转述、跨句转述、客户要求、引号内“我们决定”、明确 owner
二次拍板和直接客户发言 6 类回归。本地全量 `282 passed`，强制 source-tree
smoke 与 diff check 通过。PR #54 的 Linux Python 3.10/3.11/3.12 与 Windows
Python 3.12 CI 全部通过；隐私安全的真实语料回放仍待验证。
...
**场景**：长期治理项目的 `repo-map.json`、本地 branch 和 HEAD 完全一致，
`doctor --strict` 因而通过；但该 checkout 实际已经落后公开主线多个提交。

**问题**：旧检查证明的是“当前目录符合旧配置”，不是“当前运行时仍然新鲜”。
这会让项目在使用旧能力时持续显示健康。

**修复**：`doctor` 增加 upstream ahead/behind 报告；当映射运行时落后其
upstream 时，strict 检查失败。没有 upstream 的本地独立仓库保持兼容。

**验证**：commit `3cde18f`；新增落后 upstream 回归，全量 `258 passed`。

### FLG-ITER-20260919-02：决策关系可记录，但不能回答“改了它会影响谁” [fixed — P1]

**场景**：真实长期决策链已经包含 `supersedes`、`depends_on`、`supports`
等显式关系。某项判断变化时，接手者仍需人工扫描整份账本寻找受影响项。

**处理**：新增只读 `flg impact <decision-id>`，从正式关系推导最短影响路径
和复核候选。结果只标记 `revalidation_required` 的派生建议，不修改任何正式
状态；改变、替代或提升决策仍需 Owner Gate。

**边界**：这吸收了“判断是可计算对象”的价值，但不引入新的权威状态机、
模型依赖或自动推翻机制。正式事实源仍是 `DECISIONS.md`。

**验证**：commit `3cde18f`；独立关系测试和真实 D-037/D-047/D-048 结构回放
通过，全量 `258 passed`。

### FLG-ITER-20260919-03：一个健康灯无法覆盖长期连续性 [confirmed — P1]

**场景**：真实长期项目同时出现结构无损、代码运行时陈旧、Wiki 有新增页面、
以及本轮增量尚未 closeout。单一 `doctor OK` 无法准确表达这些状态。

**判断**：连续性健康至少应分成四类：结构完整性、运行时与来源新鲜度、
知识覆盖缺口、未收口工作增量。前两类适合 CLI 确定性检查；后两类需要宿主
在工作段边界触发检查和回写。

**下一步**：先更新宿主操作约定和健康摘要，不增加新的总状态机。跨宿主自动
触发仍保留为 P1 验证项。

### FLG-ITER-20260919-04：旧计划重新分级 [confirmed]

- Wiki continuity 已由 PR #48 合入，存储与变更检测收口；宿主触发未收口。
- Pending captures 已进入当前 Manifest，默认恢复遗漏在 CLI 层收口。
- 过期 action/review date 仍不会被通用时间规则识别，保留 P1。
- 中文 owner 范围收拢、历史/条件/未来句和说话人归因仍可能静默漏提取或误提取，
  保持 P0，优先于新增抽象层。
...
### FLG-ITER-20260919-06：决策关键词缺少时间与假设边界 [fixed — P0 slice]

**场景**：中文 closeout 中，“当时我们确认采用 A”和“如果以后确认采用 A”
都包含强决策关键词，但前者只是历史报告，后者只是条件表达。两者都不应改变
当前项目状态。相反，“之前采用 A，但现在改成 B”确实是状态迁移。

**修复**：在关键词提取前增加确定性的非当前语句检查，过滤纯历史报告、假设句
和试探性未来选项，同时保留带明确现在转换的过去—现在对比。

**验证**：3 个正反回归加入 closeout 测试；全量 `262 passed`，smoke 通过。
说话人转述和更复杂的跨句时态仍保留为后续验证，不将本次纵切宣称为完整语义理解。
