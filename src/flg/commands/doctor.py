...
def _runtime_identity(root: Path) -> dict | None:
    ...
    except (OSError, KeyError, json.JSONDecodeError, RuntimeError, subprocess.TimeoutExpired) as exc:
        return {
            "configured": True,
            "error": str(exc),
    ...
    if expected_branch and branch != expected_branch:
        issues.append(f"branch mismatch: expected {expected_branch}, found {branch}")
...
def doctor(
    ...
    overall_ok = report["status"] == "ok" and not identity_issues
    ...
    if identity is None:
        ...
        table.add_row("Runtime identity", f"unavailable: {identity['error']}")
    else:
        ...
        table.add_row("Runtime worktree", "clean" if not dirty_count else f"dirty ({dirty_count} change(s))")
    console.print(table)
