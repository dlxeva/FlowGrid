...
它把项目记忆放在明文文件里，让不同本地 Agent 能围绕同一个目录继续工作，并把决策、进展、待审核 patch 都当作正式项目状态的一部分。
...
- **支持 Agent 接力：** 后来的 Agent 读取同一套账本和 pending patches
...
FlowGrid（技术简称 FLG）是一套本地项目目录协议 + AI 工作流工具，面向策略、营销、运营、调研、解决方案等非编码知识工作。
